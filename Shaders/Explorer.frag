/*
  Original shader: https://www.shadertoy.com/view/lsGyDt
  Author: Virgill
  Licensed under a CC BY-NC-SA 3.0 license. (https://creativecommons.org/licenses/by-nc-sa/3.0/deed.en_US)
*/

#define iTimeLooping (iTime - float((int(iTime) / 140) * 140))
...
#endif    
	float s= sdCone(schein,normalize(vec2(1,1)))/length(schein*schein)+0.2*noise(10.*p+2.*sin(.2*iTimeLooping))+0.1*noise(p*23.+sin(0.3*iTimeLooping))+0.2*rnd(p.xy);
...
// scene: wobble floor
    if (scene==2)
    {
    	d=p.y-0.01*noise(50.*p)+0.4*noise(2.*p+0.2*iTimeLooping)+0.09*noise(7.*p);
    }
...
// scene: room with stone
	if (scene==3)
    {
		...
		d = min(d,length(p)-0.4-0.04*noise(p*70.)-0.5*noise(p*4.+iTimeLooping)-0.02*noise(p*114.+iTimeLooping));
...
// scene: wtf
	...
	{
        p.z+=3.5;
        p.y-=0.1*(iTimeLooping-120.);
        p.z-=0.1*(iTimeLooping-120.);
            d = .5*mapy(p);
    }   
...
float castRay(in vec3 ro, in vec3 rd, in float maxt, in vec2 co) {
    float precis = 0.001;
    float h = precis * 2.0;
    float t = -3.5+rnd(co+0.01*iTimeLooping)*7.;
    for(int i = 0; i < 200; i++) 
    {
...
// scene handling   
   	lightscene_idx 	= int(floor((iTimeLooping+.8)/lightduration));
    scene 			= int(floor(iTimeLooping/sceneduration));
	...
	lightscene_interpolated = mix(lightscene,lightscene_old, exp(-64.*pow(fract((iTimeLooping+.8)/lightduration),4.)))*0.7;

	// camera setup (iq)   
    vec2 uv = fragCoord.xy / iResolution.xy;
    vec2 p = uv * 2.0 - 1.0;
    p.x *= iResolution.x / iResolution.y;
    float theta = iTimeLooping * 3.141592 * 0.20;
    ...
    lightpos = vec3(ypos,0.7 + 0.2 * sin(theta*2.0),1.-0.2*sin(0.2*iTimeLooping)); 
...
// feedbak noise (with buffer a)
   	/*uv.x-=0.005*noise(uv.yxx*32.-iTimeLooping)-0.0025;
   	uv.y+=0.005*noise(uv.yxx*32.+iTimeLooping)-0.0025; 
...
// particles   
    ...
    {
    	float fi =float(i);
    	vec2 pos = vec2(.5+.2*sin(fi+.5*iTimeLooping+ cos(.01*iTimeLooping*fi))+ .2*sin(-fi),  
                        .5+.2*cos(fi+.2*iTimeLooping+ sin(.02*iTimeLooping*fi)))+.2*cos( fi);
    	d += clamp(1.-(length(350.*(.8+.5*sin(fi*iTimeLooping))*(pos - uv))),0.,1.);
   	}
...
// color for material
  ...
  float fade =min (3.*abs(sin((3.1415*iTimeLooping/sceneduration))),1.); 
 if (iTimeLooping>140.) fade=0.;
